<div class="ia-item-view {$item.status}{if $item.sponsored} ia-item-sponsored{/if}{if $item.featured == 1} ia-item-featured{/if}" id="article_{$item.id}">
    <div class="ia-item-view__info">
        ...
        <span class="ia-item-view__info__item"><span class="fa fa-folder"></span> <a href="{$category.link}">{$category.title|escape}</a></span>
    ...
    <ul class="list-inline share-buttons">
        ...
        {/if}
        <li><a href="mailto:?subject={$item.title|escape}&body={$item.body|strip_tags|truncate:250:'...'}:{$smarty.const.IA_SELF|escape:'url'}" target="_blank" title="Email"><i class="fa fa-envelope-square fa-2x"></i></a></li>
    </ul>
        ...
        <ul class="pager">
            {if !empty($item.prev_article)}
                <li class="pull-left text-small">
                    <a href="{ia_url item='articles' data=$item.prev_article type='url'}" title="{$item.prev_article.title|escape}">&larr; {lang key='previous_article'}</a>
                </li>
            {/if}
            {if !empty($item.next_article)}
                <li class="pull-right text-small">
                    <a href="{ia_url item='articles' data=$item.next_article type='url'}" title="{$item.next_article.title|escape}">{lang key='next_article'} &rarr;</a>
                </li>
            {/if}
        </ul>
    {/if}
