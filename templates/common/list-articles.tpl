<div class="ia-item ia-item--border ia-item--{$listing.status}{if $listing.sponsored} ia-item--sponsored{/if}{if $listing.featured} ia-item--featured{/if} has-panel" id="article-{$listing.id}">
	{if $listing.image}
		<a class="ia-item__image" href="{ia_url type='url' item='articles' data=$listing}">
			{ia_image file=$listing.image title=$listing.title class='img-responsive'}
		</a>
	{/if}
