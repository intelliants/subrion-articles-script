...
(function($) {
	$.fx.step.clip = function(fx){
		if ( fx.state == 0 ) {
			var cRE = /rect\(([0-9\.]{1,5})(px|em) ([0-9\.]{1,5})(px|em) ([0-9\.]{1,5})(px|em) ([0-9\.]{1,5})(px|em)\)/;
	...
	$.fn.jCal = function (opt) {
		return $.jCal(this, opt);
	};
	$.fn.jCalSelect = function (day, days, day_callback) {
		return $.jCalSelect(this, day, days, day_callback);
	};
	$.jCalSelect = function(target, day, days, day_callback){
		var dCursor = new Date( day.getTime() );
		for (var di=0; di < days; di++) {
			var currDay = $(target).find('[id*=d_' + ( dCursor.getMonth() + 1 ) + '_' + dCursor.getDate() + '_' + dCursor.getFullYear() + ']');
			if (currDay.length) day_callback(currDay);
	...
	$.jCal = function (target, opt) {
		...
		if(id && $.jCalData[id] && !opt.rebuild)
		{
			data = $.jCalData[target.attr('id')];
		}
		opt = $.extend({
			redraw:         false,
			percent:        false,
			...
			change:		    function (day) { return true; },		    // callback function for click on left/right button
			...
			percent_already:false,
			dow:			(intelli.lang.calendar_dow ? intelli.lang.calendat_dow.split(',') : ['S', 'M', 'T', 'W', 'T', 'F', 'S']),
			ml:		    	(intelli.lang.calendar_ml ? intelli.lang.calendat_ml.split(',') : [intelli.lang.month1, intelli.lang.month2, intelli.lang.month3, intelli.lang.month4, intelli.lang.month5, intelli.lang.month6, intelli.lang.month7, intelli.lang.month8, intelli.lang.month9, intelli.lang.month10, intelli.lang.month11, intelli.lang.month12]),
			ms:		    	(intelli.lang.calendar_ms ? intelli.lang.calendat_ms.split(',') : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']),
		...
		for (var sm=0; sm < opt.showMonths; sm++)
			$(target).append('<div class="jCalMo" style="width:'+(opt.width*7+14)+'px;"></div>');
		opt.cID = 'c' + $('.jCalMo').length;
		if(id === null)
		{
			target.attr('id', opt.cID);
		}
		if(opt.redraw && $(target).data('current'))
		{
			opt.day = $(target).data('current');
		}

		if(opt.percent)
		{
			if(!opt.percent_already)
			{
			opt.percent_already = true;
			function resize_jcal(target)
			{
				x = ($(target).parent().width() - 14) / 7;
				if(x < 20) x = 20;
				if(x > 120) x = 120;
				return Math.floor(x);
			}
			...
			opt.timeout = setTimeout((function(){
				window.clearTimeout(opt.timeout);
				x = resize_jcal(target);
				$.jCal(target, {width:x, height: x, redraw: true});
			}), 100);
			*/
			$(window).resize(function() {
				window.clearTimeout(opt.timeout);
				$.jCal(target, {width: 20, height: 20});
				x = resize_jcal(target);
				$.jCal(target, {width: x, height: x, redraw: true});
			});
			}
		...
		if(id) $.jCalData[id] = opt;
		return opt;
	};
	function drawCalControl (target, opt) {
		$(target).parent().data('current', opt.day);
		opt.change( opt.day );
		$(target).append(
			'<div class="jCal" style="width:'+(opt.width*7+14)+'px;">' +
				...
				.bind('click', $.extend( {}, opt ),
					function (e) {
						...
						var monthName = $(this).hasClass('monthName'),
							...
							calcTop = ( ($(this).offset()).top - ( ( monthName ? e.data.day.getMonth() : 4 ) * ( $(this).height() + 0 ) ) );
						calcTop = calcTop > 0 ? calcTop : 0;
						var topDiff = ($(this).offset()).top - calcTop;
						$('<div class="monthSelectorShadow" style="' +
							'top:' + target.offset().top + 'px; ' +
							'left:' + target.offset().left + 'px; ' +
							'width:' + ( target.width() + ( parseInt(target.css('paddingLeft')) || 0 ) + ( parseInt(target.css('paddingRight')) || 0 ) ) + 'px; ' +
							'height:' + ( target.height() + ( parseInt(target.css('paddingTop')) || 0 ) + ( parseInt(target.css('paddingBottom')) || 0 ) ) + 'px;">' +
						'</div>')
						...
						for (var di = ( monthName ? 0 : -4 ), dd = ( monthName ? 12 : 5 ); di < dd; di++)
						...
						$(moSel).css('border', '1px solid black')
							.css('clip','rect(' + diffOff + 'px ' + ( $(this).width() + 2 + ( pad * 2 ) ) + 'px '+ diffOff + 'px 0px)')
							.animate({'opacity':.92,'clip':'rect(0px ' + ( $(this).width() + 2 + ( pad * 2 ) ) + 'px ' + ($(moSel).height() + 2) + 'px 0px)'}, 'slow', function () {
						...
						$('.jCalMo .monthSelect', e.data._target).bind('mouseover mouseout click', $.extend( {}, e.data ),
							function (e) {
								if ( e.type == 'click' )
									target.jCal( $.extend(e.data, {day:new Date($(this).attr('id').replace(/_/g, '/'))}) );
								else
		...
		$(target).find('.jCal .left').bind('click', $.extend( {}, opt ),
			function (e) {
				...
				$(e.data._target).prepend('<div class="jCalMo" style="width:'+(opt.width*7+14)+'px;"></div>');
		...
		$(target).find('.jCal .right').bind('click', $.extend( {}, opt ),
			function (e) {
				...
				$(e.data._target).append('<div class="jCalMo" style="width:'+(opt.width*7+14)+'px;"></div>');
	...
	function reSelectDates (target, day, days, opt) {
		...
		if ( fC && typeof opt.callback == 'function' ) opt.callback( day, days, true );
	};
	function drawCal (target, opt) {
		for (var ds=0, length=opt.dow.length; ds < length; ds++)
			$(target).append('<div class="dow" style="width:'+opt.width+'px">' + opt.dow[ds] + '</div>');
		...
		var style = ' style="width:'+opt.width+'px;height:'+opt.height+'px;"';
		for ( var d = offsetDayStart, dE = ( copt.fd + copt.ld + ( 7 - offsetDayEnd ) ); d < dE; d++)
		{
			var dayResult = opt.dCheck( new Date( (new Date( fd.getTime() )).setDate( d - ( copt.fd - opt.dayOffset ) ) ) );
			$(target).append(
				(( d <= ( copt.fd - opt.dayOffset ) ) ?
					'<div id="' + opt.cID + 'd' + d + '" class="pday"'+style+'>' + ( copt.lld - ( ( copt.fd - opt.dayOffset ) - d ) ) + '</div>'
					: ( ( d > ( ( copt.fd - opt.dayOffset ) + copt.ld ) ) ?
						'<div id="' + opt.cID + 'd' + d + '" class="aday"'+style+'>' + ( d - ( ( copt.fd - opt.dayOffset ) + copt.ld ) ) + '</div>'
						: '<div id="' + opt.cID + 'd_' + (fd.getMonth() + 1) + '_' + ( d - ( copt.fd - opt.dayOffset ) ) + '_' + fd.getFullYear() + '" class="' +
							(dayResult ? 'day' : 'invday') +
							'"'+style+'>' + ( d - ( copt.fd - opt.dayOffset ) ) + (dayResult ? dayResult : '') + '</div>'
					)
				)
			);
		}
		...
		$(target).find('div[id^=' + opt.cID + 'd_]:not(.invday)').bind("mouseover mouseout click", $.extend( {}, opt ),
			function(e){
				...
				if (e.type == 'click') {
					e.data.day = osDate;
					e.data.callback( osDate, di, false );
